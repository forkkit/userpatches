...
Main() {
	case $RELEASE in
		...
		buster)
			...
			cp /tmp/overlay/cron.sh /root/cron.sh
			chmod 755 /root/cron.sh

			# set configs for our board
			cp /tmp/overlay/extlinux.conf /boot/extlinux/extlinux.conf
			cp /boot/u-boot-s905x-s912 /boot/u-boot.ext

			# set crontab to test for internet every minute and configure deviceplane when connected
			echo "* * * * * bash /root/cron.sh" > /root/cronjob
